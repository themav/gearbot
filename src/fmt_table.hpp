...
public:
  ...
  // Add a row directly to the table
  ...
  std::string print_table() {
    ...
    size_t col_count = _table[0].size();
    ...
    for (auto row : _table) {
      ...
      output << _bar << std::endl; // Trailing edge of cell and new line.
...
private:
  ...
  size_t row_width(unsigned int row_idx) {
    ...
    for (auto row : _table) {
      ...
      } catch (std::out_of_range &) {} // Do nothing because we already have a value if the field is blank.
    }
    return _width;
  }
};
